...
export default class FastShipping {
    ...
    protected _options: FastShippingOptions = {
        ...
        timerSelector: '.cs-indicators__fast-shipping-today',
        timerVariant: 'time',
        timeNotation: '24h',
    ...
    public constructor($element: JQuery, options?: FastShippingOptions) {
        ...
        this._deliveryData = {
            time: parseInt(this._$element.find(`.${this._options.namespace}indicators__fast-shipping-data-time`).val(), 10),
            nextDayTime: parseInt(this._$element.find(`.${this._options.namespace}indicators__fast-shipping-data-time-next`).val(), 10),
            deliveryDay: this._$element.find(`.${this._options.namespace}indicators__fast-shipping-data-delivery-day`).val(),
        };
    ...
    protected _getTranslationsJSON(): any {
        const $labelsInput: any = this._$element.find(
            `.${this._options.namespace}indicators__fast-shipping-labels`
        );
    ...
    protected _getTimeRemaining(): object {
        const timeRemaining: number = this._deliveryData.time - (Math.floor(Date.now() / 1000) - new Date().getTimezoneOffset() * 60);
        ...
        return {
            ...
            totalNextDay: this._deliveryData.nextDayTime - (Math.floor(Date.now() / 1000) - new Date().getTimezoneOffset() * 60),
        };
    }
    ...
    protected _getFormattedTimeTo(unixTime: number): string {
        const deadlineTimestamp: any = new Date((this._deliveryData.time + (new Date().getTimezoneOffset() * 60)) * 1000);
