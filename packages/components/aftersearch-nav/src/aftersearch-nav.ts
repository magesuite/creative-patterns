...
export default class Aftersearch {
    ...
    public constructor($element: JQuery) {
        ...
        this._$toggleButton = this._$element.find('.cs-aftersearch-nav__show-more-trigger');
        this._$toggleButtonTextSpan = this._$element.find('.cs-aftersearch-nav__show-more-trigger-text');
        this._$listOfFilters = this._$element.find('.cs-aftersearch-nav__filter');

        this._widthOfElement = this._$listOfFilters.length ? $(this._$listOfFilters[0]).outerWidth() : 0;
        ...
        $.each($filtersRows, function (i :number, e :any): void {
            ...
            if (
                ...
                $(this)
                    ...
                    .addClass('cs-aftersearch-nav__filter-content--extremely-right');
            }
        });
    }
    ...
    protected _toggleMoreFilters(): void {
        ...
        if(!this._allFiltersVisible) {
            ...
            this._$toggleButtonTextSpan.text(this._$toggleButton.data('hide-text'));
            this._$toggleButton.addClass('show');
        } else {
            this._$element.removeClass(this._filtersExpandedClass);
            this._$toggleButtonTextSpan.text(this._$toggleButton.data('show-text'));
            this._$toggleButton.removeClass('show');
        }

        this._allFiltersVisible = !this._allFiltersVisible;

        const openFilters: JQuery = $('.cs-aftersearch-nav__filter-title[aria-expanded="true"]');
    ...
    protected _attachEvents(): void {
        ...
        this._eventListeners.onFilterClick = ( event: Event ): void => {
            const $filterToOpen: JQuery = $( event.target ).hasClass( 'cs-aftersearch-nav__filter' ) ? $( event.target ) : $( event.target ).closest( '.cs-aftersearch-nav__filter' );
        ...
        this._eventListeners.onProductsGridHover = ( event: Event ): void => {
            const $hoveredProduct: JQuery = $( event.target ).hasClass( 'cs-grid-product' ) ? $( event.target ) : $( event.target ).closest( '.cs-grid-product' );
        ...
        $('.cs-grid-product').on('mouseenter mouseleave', this._eventListeners.onProductsGridHover);
